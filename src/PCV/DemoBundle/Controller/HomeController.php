...
class HomeController extends Controller
{
    ...
    */
    ...
    /**
    * @Route("/registro", name="_demo_signup")
    * @Template()
    */
    public function signupAction(Request $request){
        //redirecciona si estamos logeados
        if($request->getSession()->get('name')){
            return $this->redirect($this->generateUrl('_private_resume_profile'));
        }
        ...
        {
            $username = ucwords(strtolower(trim($request->get('_name'))));
    ...
    /**
     * @Route("/find_email", name="_find_email")
     */
    public function findEmail(Request $request){   
        $email = $request->get('email');

        $em = $this->getDoctrine()->getManager();
        $user = $em->getRepository('PCVDemoBundle:User')->findByEmail($email);
    ...
    private function sendMessage($name, $email){
        $message = \Swift_Message::newInstance()
                ...
                ->setBody(
                    $this->renderView(
                        'PCVDemoBundle:Contenido:emailregistro.html.twig',
                        array('name'=> $name)
                    ),'text/html'
                );
